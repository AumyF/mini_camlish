...
let check predicate parser =
  let* value = parser in
  let inner_fn =
    if predicate value then fun input -> Ok (value, input)
    else fun _ -> parse_err "value did not satisfy the predicate"
  in
  inner_fn

let get_identifier =
  let is_reserved_word = function
    | "in" | "if" | "then" | "else" | "let" | "fun" -> false
    | _ -> true
  in
  check is_reserved_word get_identifier
...
and get_apply i =
  (let+ fn = expr_1 and+ _ = many match_space and+ argument = expr_1 in
   Expression.Apply (fn, argument))
    i
